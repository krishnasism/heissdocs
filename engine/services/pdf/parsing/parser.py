    ...
    def _get_ocr_body(self, path, document_progress: dict) -> str:
        ...
        with Pool(processes=5, initargs=(counter)) as pool:
            page_results = [pool.apply_async(
                self._parse_pytesseract, (page_chunk, document_progress,)) for page_chunk in pages_chunks]
            pool.close()
            pool.join()
...
def get_pdf_body(pdf_file: NamedTemporaryFile, original_file_name: str, store_files_in_cloud: bool, bucket_name: str, document_progress: dict) -> dict:
    file_metadata = {
