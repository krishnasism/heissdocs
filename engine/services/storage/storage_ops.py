...
def process_s3_files(bucket_name: str):
    ...
    try:
        for s3_object in bucket.objects.all():
            ...
            temp_file.close()
            file_metadata = {"filename": path + "/" + filename}
            pdf_body, file_metadata = get_pdf_body(temp_file, filename=filename)
        return True
...
def upload_file_to_blob(filestream, filename, bucket=None):
    ...
    try:
        if ".pdf" in filename:
            ...
            response = pdf_object.put(Body=filestream, ContentType="application/pdf")
        else:
            pdf_object = s3_client.Object(bucket, filename)
