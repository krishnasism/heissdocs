...
@router.get("/search")
async def pdf_search(query: str, user_email: str, authenticated: bool = Depends(verify_token)):
    query = query.lower()
...
async def get_file_url(bucket_name: str, file_name: str, user_email: str, authenticated: bool = Depends(verify_token)):
    try:
