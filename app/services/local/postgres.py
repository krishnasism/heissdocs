...
class PostgresManager():
    ...
    def get_documents_progress(self, user_email: str, document_id=None):
        with self.client.connect() as conn:
            if not document_id:
                result_set = conn.execute(documents_progress_table.select().where(
                    documents_progress_table.c.user_email == user_email))
            else:
                result_set = conn.execute(documents_progress_table.select().where(and_(
                    documents_progress_table.c.user_email == user_email, documents_progress_table.c.document_id == document_id)))
    ...
    def create_documents_progress_entry(self, documents_progress: dict):
        documents_progress = self._convert_dict_to_snake_case(
            documents_progress)
        documents_progress['id'] = str(uuid4())
        with self.client.connect() as conn:
            statement = insert(documents_progress_table).values(
                **documents_progress)
            conn.execute(statement)
            conn.commit()
    ...
    def update_documents_progress(self, documents_progress: dict):
        ...
        stored_documents_progress = self.get_documents_progress(
            documents_progress['user_email'], documents_progress['document_id'])[0]
        with self.client.connect() as conn:
            statement = update(documents_progress_table).where(and_(documents_progress_table.c.user_email ==
                                                                    documents_progress['user_email'], documents_progress_table.c.document_id == documents_progress['document_id'])).values(
                **{
                    'stage': documents_progress['stage'] if documents_progress['stage'] else stored_documents_progress['stage'],
                    'pages_parsed': documents_progress['pages_parsed'] if documents_progress['pages_parsed'] else stored_documents_progress['pages_parsed'],
                    'total_pages': documents_progress['total_pages'] if documents_progress['total_pages'] else stored_documents_progress['total_pages']
                }
            )
            conn.execute(statement)
            conn.commit()
